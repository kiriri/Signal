...
export function flattenClassesTransformer(program: ts.Program): ts.TransformerFactory<ts.SourceFile>
{
    ...
    return (context) =>
    {
        ...
        function flattenClass(cls: ts.ClassDeclaration, file: ts.SourceFile): ts.ClassDeclaration
        {
            ...
            const baseMemberNames = new Set(baseDecl.members.map(member=>member.name?.getText()).filter(v=>v!==undefined));
            const clsMemberNames = new Set(cls.members.map(member=>member.name?.getText()).filter(v=>v!==undefined));
            ...
            const newConstructor = createFlattenedConstructor(constructorFromBase, derivedConstructor, conflictMap);
