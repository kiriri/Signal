    ...
    _get() {
        ...
        if (subscribed_to.length <= 1 && fresh_dependency_length === subscribed_to.length) {
            if (1 === fresh_dependency_length) {
                ...
                entry.last = entry.signal.peek(), global_listeners[global_listener_index] = void 0;
            }
        } else {
...
class Reducer extends Subscribable {
    ...
    register_source(source) {
        ...
        return ref.last = this.identity_value, ref.reducer = this._self ??= new WeakRef(this), 
        ref.source = source, this.on_change(source, source.get?.(), ref), ref;
    }
